...
Thank you for your interest in PrefabLens.

PrefabLens is pre-1.0 software.
Breaking changes can land without a deprecation cycle.
Large refactors are in progress.
A pull request without prior discussion can conflict with that work.
Review capacity can be unavailable.
...
2. **Wait for the `approved` label.** When we agree to implement the change, a maintainer adds the `approved` label to the issue. That label means you can start.
...
As a result, the generated notes are already grouped and readable.
A PR title becomes a release note line with no change.
