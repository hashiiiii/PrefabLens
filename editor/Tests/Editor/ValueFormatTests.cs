...
namespace PrefabLens.Tests
{
    ...
    public class ValueFormatTests
    {
        ...
        [Test]
        public void MissingSideReadsAsDash() => Assert.AreEqual("—", ValueFormat.Format(Value.Null, Model()));
        ...
        public void BuiltinRefReadsAsObjectName() =>
            Assert.AreEqual(
                "Cube (built-in)",
                ValueFormat.Format(Ref("10202", BuiltinRefs.DefaultResourcesGuid), Model())
            );

        [Test]
        public void BuiltinGuidWithUnknownFileIdKeepsTheGuid() =>
            Assert.AreEqual(
                "guid:" + BuiltinRefs.DefaultResourcesGuid,
                ValueFormat.Format(Ref("424242", BuiltinRefs.DefaultResourcesGuid), Model())
            );
        ...
        [Test]
        public void NullReferenceReadsAsNone() => Assert.AreEqual("None", ValueFormat.Format(Ref("0", null), Model()));

        [Test]
        public void LocalRefReadsAsHashFileId() => Assert.AreEqual("#42", ValueFormat.Format(Ref("42", null), Model()));
