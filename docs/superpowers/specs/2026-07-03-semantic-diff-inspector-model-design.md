...
- **ノード移動**(`m_TransformParent` / `m_Father` の変更のみ): 変更行としては**不可視**。木は after 側の構造で組まれるためノードは移動先の位置に現れるが、これらの生 field diff は構造抑制・非表示ルールで落ち、「どこから移動してきたか」は表示されない(構造抑制ルールの既知の帰結)。
- **`m_SourcePrefab` の差し替え**(override 構成は同じまま参照先 prefab だけ変わる): override diff にも木構造にも現れず**不可視**(同上の帰結)。`sourceGuid` は after 側の値が JSON に出るため、guid 解決済みなら ‹Prefab› バッジのパス表示だけが変わる。
