#!/usr/bin/env bash
# Tests for render.sh: it renders the Homebrew formula from prefablens.rb,
# substituting the version and the per-target SHA256 hashes of the
# release zips.
set -euo pipefail
cd "$(dirname "$0")"
...
# The version reaches the formula.
grep -q 'version "1.2.3"' "$tmp/out/prefablens.rb" || fail "formula version"

# The formula embeds the hash of each target's asset.
...
if grep -q '{{' "$tmp/out/prefablens.rb"; then
