...
/.github/workflows/ci.yml        # install toolchain via mise; test + perf jobs (parallel)
/core/
  src/
    ...
    perf.zig                     # synthetic scene builder + timeDiff (Task 13)
    perf_main.zig                # `zig build perf` budget-gate entrypoint (Task 13)
/cli/
  src/
...
各 `src/*.zig` ファイルは 1 つの責務を担い、それ自身の `test {}` ブロックを含みます（Zig の慣習）。`root.zig` と `main.zig` が `build.zig` に組み込まれる 2 つのテストルートです。フィクスチャは Phase 1 ではインライン文字列リテラルと `std.testing.tmpDir` で賄い、コミット済みの fixtures ディレクトリは作りません（大規模シーンも Task 13 でメモリ上に合成します）。
...
- [x] **Step 2: コンパイルに失敗することを確認するために実行する**
...
- [x] **Step 1: 最もシンプルなケース（ドキュメント分割 + フラットマップ）の失敗するテストを書く**
...
- [x] **Step 2: 実行 — 失敗を確認する（`parse` が存在しない）**

実行: `zig build test`
期待される結果: FAIL — `use of undeclared identifier 'parse'`。

- [x] **Step 3: 行モデル、ドキュメント分割、ブロックマッピングを実装する**
...
- [x] **Step 4: フロー値と参照(ref)の失敗するテストを書く**
...
- [x] **Step 5: `parseValue`（フローマップ／フローシーケンス、参照(ref)、スカラー）を実装する**
...
- [x] **Step 2: `root.zig` に `diffBytes` を追加し、実行して失敗を確認する**
...
- [x] **Step 2: re-export と `diffToJson` を追加し、実行して失敗を確認する**
...
- [x] **Step 1: 引数パースと `--json` のエンドツーエンドの失敗するテストを書く**
...
- [x] **Step 2: 失敗することを確認するために実行する**

実行: `zig build test`
期待される結果: FAIL — `parseArgs` / `Format` / `run` が未宣言。

- [x] **Step 3: 引数パース、ファイル読み込み、`run` を実装する**
...
- [x] **Step 4: リゾルバのテストを実行する**

実行: `zig build test`
期待される結果: PASS。

- [x] **Step 5: リゾルバを `--json` 出力に配線する**
...
- [x] **Step 2: import を接続し、失敗を確認するために実行する**
...
- [x] **Step 5: CLI に git-ref モードを追加する**
...
```
> このブランチは、`--git` の*前に*パースされた `--json`/`--html`/`--project` が依然として適用されるように配置すること (これらは返される構造体で捕捉される `format`/`project_root` を設定する)。`--git` の後のフラグはそのオペランドとして扱われる。`--git` は必ず最後に来なければならないことをドキュメント化すること。あわせて `run` の usage 文字列も `usage: prefablens [--json|--html] [--project DIR] (<before> <after> | --git <beforeRef> <afterRef> <path>)` に更新すること。
...
- [x] **Step 1: 失敗するテストを書く (安定した golden のため色は無効)**
...
- [x] **Step 4: レンダラのテストを実行する**

実行: `zig build test`
期待される結果: PASS。

- [x] **Step 5: `main.zig` でレンダラを使用する**
...
- [x] **Step 1: perf ハーネスをテストとして記述する（まず小さい予算でゲートを実証する）**
...
- [x] **Step 2: 再エクスポートして実行し、テストがパスすることを確認する**
...
- [x] **Step 3: リリースモードの `perf` 実行ファイルとビルドステップを追加する**
...
- [x] **Step 4: perf を CI に組み込む**

`.github/workflows/ci.yml` に `perf` ジョブを追加する（`test` ジョブと `needs` なし — runner が空いていれば並列実行）:
```yaml
  perf:
    runs-on: ubuntu-latest
    steps:
      ...
      - name: Performance budget
        run: zig build perf
```
`test`（Debug）と `perf`（ReleaseFast）は optimize が異なるため別ジョブに分ける。GitHub のチェックも「正しさ」と「性能退行」で分離される。
